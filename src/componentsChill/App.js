...
const App = () => {
   ...
   */
  useEffect(() => {
    const storedIndex = localStorage.getItem('paroldle_chill_index');
    ...
    const storedGuessList = localStorage.getItem(`paroldle_chill_guessList_${storedIndex}`);
    if (storedGuessList) {
      setGuessList(JSON.parse(storedGuessList));
    }

    const storedFoundSongs = localStorage.getItem('paroldle_chill_foundSongs');
    if (storedFoundSongs) {
      setFoundSongs(JSON.parse(storedFoundSongs));
    }

    const storedAutoplay = localStorage.getItem('paroldle_chill_autoplay');
    if (storedAutoplay) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('paroldle_chill_index', index);
    const storedGuessList = localStorage.getItem(`paroldle_chill_guessList_${index}`);
  ...
  useEffect(() => {
    if (index == null) return;
    localStorage.setItem(`paroldle_chill_guessList_${index}`, JSON.stringify(guessList));
    // eslint-disable-next-line
  }, [guessList]);

  // Sauvegarde automatique des chansons trouvées
  useEffect(() => {
    localStorage.setItem('paroldle_chill_foundSongs', JSON.stringify(foundSongs));
  }, [foundSongs]);

  // Sauvegarde automatique de l'autoplay
  useEffect(() => {
    localStorage.setItem('paroldle_chill_autoplay', autoplay);
  }, [autoplay]);
  ...
  const resetDB = () => {
    Object.keys(localStorage)
      .filter((key) => key.startsWith('paroldle_chill_'))
